                  ...
                  <tr key={c.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="p-3">
                      <div className="flex gap-1">
                        {c.certStatus === 'PENDIENTE' && <button onClick={async () => { try { const user = JSON.parse(localStorage.getItem('user') || '{}'); await api.certification.validate(c.id, { userId: user.id }); load(); } catch { alert('Error al validar'); } }} className="text-xs bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600">Validar</button>}
                        {c.certStatus === 'VALIDADO' && <button onClick={async () => { try { const user = JSON.parse(localStorage.getItem('user') || '{}'); await api.certification.certify(c.id, { userId: user.id }); load(); } catch { alert('Debe tener coordenadas'); } }} className="text-xs bg-emerald-500 text-white px-2 py-1 rounded hover:bg-emerald-600">Certificar</button>}
